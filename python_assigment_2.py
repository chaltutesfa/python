    ...
    try:
        ...
        with open("student_info_db.txt", 'a+') as file:
            for i in range(num_students):
                ...
                while True:
                    st_gen_input = input("Enter student gender: ")
...
def top_students_per_department():
    try:
        with open("student_info_db.txt", "r") as file:
            ...
            if max_name:
                print(f"Top scoring student in {st_dep.capitalize()} department: {max_name} with GPA {max_gpa}")
            else:
                print(f"No student found in {st_dep.capitalize()} department.")
    except Exception as e:
        print(f"An error occurred: {e}")
